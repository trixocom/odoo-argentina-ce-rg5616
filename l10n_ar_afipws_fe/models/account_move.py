...
class AccountMove(models.Model):
    ...
    def do_pyafipws_request_cae(self):
        ...
        for inv in self:
            ...
            if not condicion_iva_receptor_id:
                raise UserError(_('El cliente no tiene definida la Condición frente al IVA (obligatoria según RG 5616).'))
            _logger.info('Condicion IVA Receptor ID: {}'.format(str(condicion_iva_receptor_id)))
            _logger.info(str(type(condicion_iva_receptor_id)))
            ...
            if afip_ws == 'wsfe':
                ws.CrearFactura(
                    ...
                    moneda_id, moneda_ctz, condicion_iva_receptor_id=condicion_iva_receptor_id,
                )
                _logger.info("Factura generada: %s", ws.factura)
            ...
            #     )
                ...
                ws.CrearFactura(
                    ...
                    idioma_cbte, incoterms_ds, fecha_pago, condicion_iva_receptor_id,
                )
            elif afip_ws == 'wsbfe':
                ...
                ws.CrearFactura(
                    ...
                    imp_perc_mun, imp_internos, moneda_id, moneda_ctz, condicion_iva_receptor_id,
                    fecha_venc_pago
                )
            ...
            try:
                if afip_ws == 'wsfe':
                    ws.CAESolicitar()
                    _logger.info(ws.client.xml_request)
                    vto = ws.Vencimiento
                elif afip_ws == 'wsmtxca':
